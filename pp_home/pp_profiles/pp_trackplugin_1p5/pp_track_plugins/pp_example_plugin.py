...
API
----
Track plugins should be in the profile in the directory /pp_track_plugins or in  /pipresents/pp_track_plugins. Plugins are organised as Python Classes.
...
------
  ...
  load has the following arguments:
    ...
    False if the plugin is called from any other type of show. These shows have medilaist and track_file and type are provided by the track entry in the medialist and 
...
class pp_example_plugin(object):
    ...
    def load(self,track_file,liveshow,track_type):
        ...
        if self.liveshow==True:
            # liveshows have a limited number of track types image, mpv, audio. Plugins can add message to liveshows
            ...
            elif self.track_type=='chrome':
                # let's try a different url
                self.modified_file='http://www.museumoftechnology.org.uk'
            else:
